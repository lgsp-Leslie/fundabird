...
def check_email():
    # 重新发送验证邮件
    send_email_obj = SendEmail.query.filter_by(use_type='REGISTER').first()
    send_email(g.user.id, send_email_obj.sender, send_email_obj.subject, g.user.email,
               '/foreground/accounts/verification_email.html')
    return redirect(url_for('mall.index'))
...
def modify_email():
    ...
    if user_obj:
        ...
        db.session.commit()
        send_email_obj = SendEmail.query.filter_by(use_type='REGISTER').first()
        send_email(user.id, send_email_obj.sender, send_email_obj.subject, user.email,
                   '/foreground/accounts/verification_email.html')
