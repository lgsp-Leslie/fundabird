...
class Product(BaseModel):
    ...
    product_id = db.Column(db.String(128), nullable=False, comment='商品id，（团购：#GD-；创新：#IV-；基金：#FD-。）')
...
class InnovationPosterList(BaseModel):
    # 众筹商品与众筹者关联表
    __tablename__ = 'mall_innovation_poster_list'

    user_id = db.Column(db.Integer, db.ForeignKey('mall_user.id'))
    user = db.relationship('User', backref='innovation_poster_list')

    product_id = db.Column(db.Integer, db.ForeignKey('mall_product.id'))
    product = db.relationship('Product', backref='innovation_poster_list')


class InnovationUpdates(BaseModel):
    # 众筹商品作者追加更新
    __tablename__ = 'mall_innovation_update'

    title = db.Column(db.String(50), nullable=False, comment='标题')
    desc = db.Column(db.String(512), nullable=False, comment='内容')

    user_id = db.Column(db.Integer, db.ForeignKey('mall_user.id'))
    user = db.relationship('User', backref='innovation_updates')

    product_id = db.Column(db.Integer, db.ForeignKey('mall_product.id'))
    product = db.relationship('Product', backref='innovation_updates')
