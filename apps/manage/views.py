...
from apps.models.manage_models import Admin, ProductAuditResult
from apps.utils.utils import upload_file, send_email
...
@manage_bp.route('/add_group_deals', methods=['GET', 'POST'])
def add_group_deals():
    uid = session.get('uid')
    if request.method == 'POST':
        ...
        if thumbnail:
            thumbnail_name = upload_file(thumbnail, Config.UPLOADS_GROUP_DEALS_THUMBNAIL_IMAGE_DIR)
            if thumbnail_name is not None:
                file_path = Config.UPLOADS_GROUP_DEALS_THUMBNAIL_IMAGE_DIR + thumbnail_name
                thumbnail.save(file_path)
                product.thumbnail = Config.UPLOADS_GROUP_DEALS_THUMBNAIL_IMAGE + thumbnail_name
            else:
...
@manage_bp.route('/modify_group_deals')
def modify_group_deals():
    pass


@manage_bp.route('/del_group_deals')
def del_group_deals():
    pass
...
@manage_bp.route('/innovation_list')
def innovation_list():
    page = int(request.args.get('page', 1))
    page_data = Product.query.filter_by(product_type_id=2).order_by(Product.created_at.desc()).paginate(page=page, per_page=Config.PER_PAGE)
    return render_template('background/product_innovation_list.html', page_data=page_data)


@manage_bp.route('/add_innovation', methods=['GET', 'POST'])
def add_innovation():
    uid = session.get('uid')
    if request.method == 'POST':
        group = request.form.get('group')
        ...
        desc = request.form.get('desc')
        content = request.form.get('content')
        current_price = int(float(request.form.get('original_price')) * 100)
        total = request.form.get('total')
        start_time = request.form.get('start_time')
        end_time = request.form.get('end_time')
        coins = request.form.get('coins')
        ...
        video_path = request.form.get('video_path')
        files = request.files.getlist('images')
        product_id = 'IV-' + str(uuid4().hex) + datetime.now().strftime('%Y%m%d%H%M%S')
        product = Product()
        product.product_id = product_id
        if thumbnail:
            thumbnail_name = upload_file(thumbnail, Config.UPLOADS_INNOVATION_THUMBNAIL_IMAGE_DIR)
            if thumbnail_name is not None:
                file_path = Config.UPLOADS_INNOVATION_THUMBNAIL_IMAGE_DIR + thumbnail_name
                thumbnail.save(file_path)
                product.thumbnail = Config.UPLOADS_INNOVATION_THUMBNAIL_IMAGE + thumbnail_name
            else:
                flash('缩略图不能为空!', 'danger')
                return redirect('manage.add_product_group_deals')
        ...
        project_type_obj = ProductTypes.query.filter_by(name='Innovation').first()
        project_type_id = project_type_obj.id
        product.product_type_id = project_type_id
        ...
        flash('新增成功', 'success')
        return redirect(url_for('manage.innovation_list'))

    return render_template('background/product_innovation_add.html')


@manage_bp.route('/audit_innovation', methods=['GET', 'POST'])
def audit_innovation():
    uid = session.get('uid')
    print('uid', uid)
    ...
    status_list = constants.ProductStatus
    ...
    if request.method == 'POST':
        ...
        audit_obj = ProductAuditResult()
        audit_obj.result = status
        audit_obj.reason = reason
        audit_obj.reviewer_id = uid
        if aid:
            audit_obj.admin_id = aid
            print('aid',aid)
            send_mail = Admin.query.get(aid)
            send_mail = send_mail.email
        elif uid:
            audit_obj.user_id = uid
            print('uid',uid)
            send_mail = User.query.get(uid)
            send_mail = send_mail.email
        audit_obj.product_id = pid
        db.session.add(audit_obj)
        db.session.commit()
        print('send_mail', send_mail)
        send_email(subject='审核结果', addressee=send_mail, template='background/email/findings_of_audit.html', result=status, reason=reason)
        flash('审核成功！', 'success')
        return redirect(url_for('manage.innovation_list'))

    return render_template('background/product_innovation_detail.html', product_obj=product_obj, i_list=i_list, v_list=v_list, status_list=status_list)
