...
# 钩子函数,检查是否登录账号
required_mall_login_list = ['/mall/edit_profile']

required_manage_login_list = ['/manage/index', '/manage/add_admin', '/manage/group_deals_list', '/manage/add_group_deals', '/manage/product_type_list', '/manage/user_list', '/manage/admin_list', '/manage/innovation_list', '/manage/add_innovation', '/manage/audit_innovation']
...
def before_request():
    ...
    if request.path in required_manage_login_list:
        username = session.get('username')
        if username is None or token is None:
            g.user = None
            return redirect(url_for('manage.login'))
        else:
            cache_set_value = cache.get(username)
            if cache_set_value:
                if token in cache_set_value:
    ...
    if not uid or not token:
        if request.path in required_mall_login_list:
