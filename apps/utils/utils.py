...
# 发送邮件
def send_email(subject, addressee, template, user_id=None, result=None, reason=None,  **kwargs):
    msg = Message(subject, recipients=[addressee])
    # 验证邮箱
    if user_id is not None:
        token = generate_password_hash(str(user_id))
        cache.set(str('confirm_' + str(user_id)), token, timeout=259200)
        msg.html = render_template(template, user_id=str(user_id), token=token)
        mail.send(msg)
        return 'Successful!'
    # 项目审核结果通知
    elif result is not None:
        msg.html = render_template(template, result=result, reason=reason)
        mail.send(msg)
        return 'Successful!'
